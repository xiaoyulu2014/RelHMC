...
comp=2 #nr comp
d=3 # dimension
w=ones(comp)/comp #equal weights
nu=5.0 # prior parameter
...
h=1.0e-3
sgldspecs=SGMCMC.SGLD_specs(h;mass=[1.0],niters=10)
ss=100000
[copy(th) for i=1:10]
SGMCMC.sgld!(th,p->LpostFW_gradient(p,nmix,w),sgldspecs)
# global fails=0
# function valid(th)
#   global fails
...
include("srock_refactor.jl")
T=5.0

function srockp(h)
  srocksp=SROCK_specs(h,30,niters=convert(Int32,ceil(T/h)))
  srock(copy(th),p->LpostFW_gradient(p,nmix,w),srocksp)
end
function euler(h)
  srocksp=SROCK_specs(h,30,niters=convert(Int32,ceil(T/h)))
  srock(copy(th),p->LpostFW_gradient(p,nmix,w),srocksp)
