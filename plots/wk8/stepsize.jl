...
using HDF5
##examine the effect of stepsize
nitersvec = round(linspace(5,100,16));ESS=SharedArray(Float64,length(nitersvec),2);accratiovec=SharedArray(Float64,length(nitersvec))
@sync @parallel for i=1:length(nitersvec)
	shmc = HMCState(zeros(2),niters=nitersvec[i],stepsize=0.05);
	hmc,accratio = run(shmc,dm,num_iterations=1000000, final_plot=false)
  arf = StatsBase.autocor(hmc)
	ESS[i,:] = [1000000/(1+2*sum(arf[:,j])) for j=1:size(hmc,2)]
...
outfile=open("hmc_niters005","a") #append to file
    println(outfile,"ESS=",ESS,"; accratiovec=",accratiovec, "; nitersvec=", nitersvec)
close(outfile)
h5open("hmc_niters005.h5", "w") do file
    write(file, "ESS", ESS,"accratiovec", accratiovec, "nitersvec", nitersvec)  # alternatively, say "@write file A"
end


rESS=SharedArray(Float64,length(nitersvec),2);raccratiovec=SharedArray(Float64,length(nitersvec))
@sync @parallel for i=1:length(nitersvec)
	srhmc = RelHMCState(zeros(2),niters=nitersvec[i],c=9.3, mass=0.4,stepsize=0.05);
	rhmc,raccratio = run(srhmc,dm,num_iterations=1000000, final_plot=false)
  arf = StatsBase.autocor(rhmc)
	rESS[i,:] = [1000000/(1+2*sum(arf[:,j])) for j=1:size(rhmc,2)]
...
outfile=open("rhmc_niters005","a") #append to file
    println(outfile,"rESS=",rESS,"; raccratiovec=",raccratiovec, "; nitersvec=", nitersvec)
close(outfile)
h5open("rhmc_niters005.h5", "w") do file
    write(file, "rESS", rESS,"raccratiovec", raccratiovec, "nitersvec", nitersvec)  # alternatively, say "@write file A"
end


plot(nitersvec,ESS,label="HMC")
plot(nitersvec,rESS,label="rHMC")
legend()
title("ESS vs number of leapfrog steps, stepsize=0.05")
